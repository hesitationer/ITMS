    ...
    else
    {
        ...
        if (speed < speedThresh)
        {
            if (!m_isStatic)
            {
                ...
                cv::putText(img, label, m_staticRect.tl(), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255));
        ...
        if (!dataCorrect)
        {
            ...
            if (!m_tracker || m_tracker.empty())
            {
                        ...
                        lastRect.area() > 0)
                {
                    ...
                    cv::rectangle(tmp, lastRect, cv::Scalar(0, 255, 255), 2); // yellow
                    cv::imshow("init", tmp);
#endif
            ...
            if (!inited && !m_tracker.empty() && m_tracker->update(cv::UMat(currFrame, roiRect), newRect))
            {
				...
				cv::Rect2d lastRect(m_predictionRect.x - roiRect.x, m_predictionRect.y - roiRect.y, m_predictionRect.width, m_predictionRect.height);
				cv::rectangle(tmp2, lastRect, cv::Scalar(0, 255, 255), 2); // yellow
                cv::rectangle(tmp2, newRect, cv::Scalar(255, 0, 255), 2);				// magenta

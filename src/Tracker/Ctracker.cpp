        ...
        )
{
    ...
    assignments_t assignment(N, -1); // existing blobs 가 현재 들어온 region의 어떤 애와 매칭이 되는 지를 정의
    ...
    if (!tracks.empty())
    {
        ...
        for (size_t i = 0; i < assignment.size(); i++)
        {
            if (assignment[i] != -1)
            {
                if (Cost[i + assignment[i] * N] > m_settings.m_distThres) // 가장 가깝다고 한 region과의 최소 거리 조건 첵크
                {
                    assignment[i] = -1;

...
#if (CV_VERSION_MAJOR < 4)
	std::vector<cv::String> outputNames =getOutputsNames(m_net);
    cv::String outputName = "yolo_23"; // was "detection_out"
#else
    cv::String outputName = cv::String();
#endif	
    cv::Mat detectionMat = m_net.forward(outputName); //compute output
    // debug
    cv::Mat debugImage = colorFrame(crop).clone();
    cv::imshow("croped image", debugImage);
    cv::waitKey(1);
